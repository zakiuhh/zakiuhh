      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12">
        {topCertifications.map((cert, idx) => (
          animationsEnabled ? (
            ...
            <div
              key={cert.title}
              className="p-6 rounded-3xl bg-bg-surface border border-border-subtle flex flex-col justify-between shadow-xl"
            >
              <div>
                <div className="flex items-center justify-between text-xs dev-tag text-text-dim mb-4">
                  <span className="flex items-center gap-1.5 text-accent font-semibold">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {additionalCerts.map((c, idx) => (
            animationsEnabled ? (
              ...
              <div
                key={c}
                className="flex items-center gap-3 text-xs text-text-sub bg-bg-elevated/70 px-4 py-3 rounded-xl border border-border-subtle/60"
              >
                <CheckCircle className="w-4 h-4 text-accent shrink-0" />
                <span className="font-medium">{c}</span>
              </div>
